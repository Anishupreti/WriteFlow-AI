...
(async()=>{
 ...
 try{
 const page=await browser.newPage(),worker=page,errors=[];
 page.on('pageerror',e=>errors.push(e.message));
 await page.goto(`${base}/review/review.html`);
 ...
 await page.goto(`${base}/review/review.html#draft=${draftId}`);await page.waitForFunction(()=>document.querySelector('#statement').value==='Original quoted evidence');
 ...
 assert.deepEqual(errors,[]); console.log('PASS: Chromium HTTP fixture (not extension integration); projects/items; XSS-safe display; status history; 12 concurrent writes; source sanitisation and provenance; duplicate capture prevention; failed mutation rollback; reload persistence; JSON export; responsive layout; no page errors.');
